...
Epoll::Epoll(int p){
	...
	if(SERVER_TASK == 1){
		...
		Agent_listen *agent_listen_pr = new Agent_listen(this->listen_fd,temp_trans_task);
	}//中继服务
    else if(SERVER_TASK == 0){
		Task_listen_echo *temp_echo_task = new Task_listen_echo;
		Agent_listen *agent_listen_pr = new Agent_listen(this->listen_fd,temp_echo_task);
	}
